var _bsap = {
    apiurl: 'http:\/\/s3.buysellads.com\/r\/',
    dz: [],
    dii: [],
    inv: [],
    ...
    exec: function() {
        ...
        for (var cl = function(cl) {
            for (var n=!!document.getElementsByClassName, ret = [], els = n ? document.getElementsByClassName(cl) 
                : document.getElementsByTagName('*'), p = n ? false : new RegExp('(^|\\s)' + cl + '(\\s|$)'), i = 0;
            i < els.length;
            i++)if (!p || p.test(els[i].className))
                ret.push(els[i]);
            return ret
        }, bs = cl('bsarocks'), id, pk, p = /bsap_([a-f0-9]+)/i, i = 0; i < bs.length && (id = bs[i].getAttribute('id')) && (rid = id.split('_')[1]) && (pk = bs[i].getAttribute('rel') || ((pk = p.exec(bs[i].className)) ? pk[1] : '')) && (bs[i].className = 'bsap_' + rid + ' bsap'); i++)
    ...
    deploy: function() {
        ...
        var writescript = null, scripts = 0, writes = {};
        function doc_write(write) {
            if (!writes[writescript.id])
                writes[writescript.id] = '';
            writes[writescript.id] += write
        }
        function flushWrite(which) {
            if (writes[which]) {
                ...
                setInnerHtmlAndExec(writer, writes[which]);
                delete writes[which]
            }
        }
        var scriptqueue = [];
        function stepQueue(which) {
            if (scriptqueue.length > 0 && scriptqueue[0].which == which&&!scriptqueue[0].run)
                scriptqueue.shift();
            runQueue()
        }
        function runQueue() {
        ...
        function findScripts(t) {
            for (var i = 0; i < t.childNodes.length; i++)
                t.childNodes[i].nodeName.toLowerCase() == 'script' && t.childNodes[i].className != 'ignoreme' ? gotScript(t.childNodes[i]) : findScripts(t.childNodes[i])
        }
        ...
        bsa.filter(z.filterby, function(f) {
            ...
            var empty = 0, exec = [], c = (!zf ||!zf.ads ||!zf.ads.length) ? []: bsa.getads(zf, z.nads), d, e = c[0], t = '', o = '', fr, a, backfilled = false, tf = z.format ? z.format: (z.type && z.type == 1 ? 2 : 0), ts = Math.round( + new Date() / 1000), w = z.width, h = z.height, css = function(x) {
            ...
            if ((z.noempties && z.noempties == 1) || bsa.oshow)
                empty = 0;
            if (t <= 1) {
            ...
            if (tf == 0) {
                if (typeof(ShowAdHereBanner) === 'undefined'&&!z.nostyle) {
                    ...
                    if (w < 100)
                        sc += 'div.bsap_' + z.id + ' a em{display:block;text-indent:-9000px}div.bsap_' + z.id + ' a{height:' + h + ';line-height:0}div.bsap_' + z.id + ' a.adhere{font-size:0}';
                ...
                for (i = j; !backfilled && i < z.nads && ah && (i < (ra ? z.nads : (j + 1))); i++, o += (bsa.callback(z.id, i-1, z.nads) 
                    || '<a href="http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=adhere&utm_content=zone_' + z.id + '" title="Advertise Here" class="adhere ad' + i + ' ' + (i%2 === 0 ? 'even' : 'odd') + '" target="_blank">Advertise Here<\/a>')) {}
                if (z.model == 1 && ah && (j == z.nads || backfilled))
                    o += '<div class="bsap_adhere' + (w == 728 ? '2' : '') + '"><a href="http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=cpmadhere&utm_content=zone_' + z.id + '">advertise here<\/a><\/div>'
            } else if (tf == 1) {
                ((z.model == 1 && Math.random() * 100 * z.nads < (empty || 0)) || c.length == 0) && ah && (c = [{
                    id: - i,
                    link: 'http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id,
                    img: bsa.apiurl + 'd\/adhere.png',
                    alt: 'Advertise Here',
                    text: 'Advertise in this spot with Online Advertising Marketplace'
                }
                ]);
                ...
                for (i = 0, a; i < c.length && (a = bsa.getads(c[i], 1)[0]);
                i++, (j++, o += '<div class="bsa_it_ad ad' + i + ' ' + (i%2 === 0 ? 'even' : 'odd') + '" id="bsa_' + a.id + '"><a ' + bsa.link(a.id, z.id, a.link.replace('[timestamp]', ts)) + ' target="_blank"><span class="bsa_it_i"><img src="' + a.img.replace('[timestamp]', ts) + '" width="' + z.width + '" height="' + z.height + '" alt="' + a.alt + '" \/><\/span><\/a><a ' + bsa.link(a.id, z.id, a.link) + ' target="_blank"><span class="bsa_it_t">' + a.alt + '<\/span><span class="bsa_it_d">' + a.text + '<\/span><\/a><div style="clear:both"><\/div><\/div>', t += a.id + ';')) {}
                if (z.cpmbackfill && j == 0) {
                    for (var r = Math.random(), bn = 0; bn < z.cpmbackfill.length && z.cpmbackfill[bn] && z.cpmbackfill[bn][0] < r; r -= z.cpmbackfill[bn++][0]) {}
                    if (z.cpmbackfill[bn] && z.cpmbackfill[bn][0] && z.cpmbackfill[bn][1].length > 0) {
                        o += '<iframe width="' + w + '" height="' + h + '" id="backfill_' + z.id + '_frame" src="' + z.cpmbackfill[bn][1] + '" frameborder="0" class="bsap_backfillframe" scrolling="no"><\/iframe>';
                ...
                for (i = 0, a; i < c.length && (a = bsa.getads(c[i], 1)[0]);
                i++, o += '<li class="bsapt_' + a.id + ' ad' + i + ' ' + (i%2 === 0 ? 'even' : 'odd') + '"><a ' + bsa.link(a.id, z.id, a.link) + ' target="_blank"><div class="bwr"><em class="bt">' + a.title + '<\/em><em class="bd">' + a.text + '<\/em><\/div><\/a><\/li>', t += a.id + ';') {}
                for (i = j; (typeof(bsa.callback) == 'function') && i < z.nads; i++, o += bsa.callback(z.id, i-1, z.nads) 
                    || '') {}
                    o += '<\/ul>';
                    if (c.length > 0)
                        o += '<div class="bsa_idb"><span class="bsa_idl"><a href="http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=textdesc&utm_content=zone_' + z.id + '" target="_blank">ads by <em>BSA<\/em><\/a><\/span><\/div><\/div><\/div>'
            } else if (tf == 4) {
            ...
            if (o&&!b.innerHTML.length) {
                ...
                if (exec.length)
                    for (i = 0; i < exec.length; i++)
                        setInnerHtmlAndExec(document.getElementById(exec[i][0]), exec[i][1])
            }
        ...
        for (i = 0; i < a.length; i++)
            b += new Array(a[i].per + 1).join(i + ',');
        b = b.substr(0, b.length-1).split(',');
        while (ret.length < n && i++<n * 100 && (c = a[b[Math.floor(Math.random() * b.length)]]))if (n === 1)
            return [c];
        else if (!got[c.id]) {
            got[c.id] = 1;
            ret.push(c)
        ...
        if (!fs)
            fn(s);
        if (fs.os && fs.os[(os = (/(win|mac|linux|iphone|blackberry|pike)/.exec(navigator.platform.toLowerCase()) || ['?'])[0])])
            s.push(os);
        if (fs.geo) {
            IAmGot.push(fn);
            ...
            IAm = function(y) {
                ...
                if (IAmGot.length > 0) {
                    s.push(y);
                    for (var li = 0; li < IAmGot.length; li++)
                        IAmGot[li](s);
                    IAmGot = []
                }
            };
            if (i >= 0)
                IAm(c.substring(i + 7 + 1).split(';')[0]);
            else {
